...
for f in fft_params:
    ...
    for alg in alg_list:
        ...
        split_dataframe_index = len(df_train) // 2
        df_train_1 = df_train.iloc[:split_dataframe_index, :]
        df_train_2 = df_train.iloc[split_dataframe_index:, :]


        mag_volume = []
        for index, row in tqdm(df_train_1.iterrows(), total=len(df_train_1)):
        ...
        df_train.at[:split_dataframe_index - 1, feat_name] = mse

        mag_volume = []
        for index, row in tqdm(df_train_2.iterrows(), total=len(df_train_2)):
            feat_path = os.path.join(train_feat_root_path, row['audio_filename'] + '.npy')
            bicoh = np.load(feat_path)
            mag = np.abs(bicoh)
            mag = (mag - np.min(mag)) / (np.max(mag) - np.min(mag))
        ...
        df_train.at[split_dataframe_index:, feat_name] = mse
    ...
    df_train.to_pickle('../features/unet_norm/train_nfft_{}_hop_size_{}.pkl'.format(nfft, hop_size))
